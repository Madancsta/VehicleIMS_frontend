...
function SalesPage() {
  ...
  return (
    <div>
      ...
      {message && (
        <div
          className={`mb-6 rounded-lg border p-4 text-sm shadow-sm ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                {bookings.map((b) => (
                  <option key={b.bookingId} value={b.bookingId}>
                    Booking #{b.bookingId} - {new Date(b.bookingDate).toLocaleDateString()} -{" "}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {parts.slice(0, 9).map((p) => (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">Stock: {p.stockQuantity}</div>
                </button>
              ))}
            </div>
            {parts.length === 0 && (
        ...
        <div className="bg-card border border-border rounded-lg p-6 h-fit sticky top-8">
          ...
          <div className="space-y-2 text-sm">
            ...
            {serviceCharge > 0 && (
              <Row
                label={`Service Charge (${selectedService?.serviceType})`}
                value={`Rs. ${serviceCharge.toLocaleString()}`}
              />
            )}
            <Row label="Subtotal" value={`Rs. ${subtotal.toLocaleString()}`} />
            <Row
              label={subtotal > 5000 ? "Loyalty Discount (10%)" : "Loyalty Discount"}
