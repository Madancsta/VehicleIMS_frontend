...
      {/* Stats Cards — Increased Icon Size (h-6 w-6) */}
...
          ['Total value', `Rs. ${parts.reduce((s, p) => s + p.stockQuantity * p.partPrice, 0).toLocaleString()}`, BarChart3, false],
...
                    {label}
                  </div>
                  <div className={`font-display text-2xl font-bold mt-2 ${hasAlert ? 'text-red-600' : ''}`}>
                    {value}
                  </div>
                </div>
                <div className="pt-1">
...
      {/* Main Content Box — Wrapping Filter and Table */}
...
                      <span className={`inline-flex items-center gap-1 ${low ? 'text-red-600 font-bold' : ''}`}>
...
// Full PartFormModal implementation restored
